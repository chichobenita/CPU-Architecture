...
### TopIO (IO_Interface.vhd)
Digital system with I/O interface for hardware testing.

### Top (top.vhd)
Wraps the entire system, routing inputs to appropriate modules and organizing outputs. this is the digital system we are connecting to IO.

### combinatorial digital circuit (combinatorial_digital_circuit.vhd)
Wraps the combinatorial modules we made at lab 1 (ALU).

### AdderSub (AdderSub.vhd)
Performs addition or subtraction between signals X and Y.
...
### synchronous digital circuit (synchronous_digital_circuit.vhd)
Wraps the synchronous part of the system (PWM).
...
### Counter (counter16bit.vhd)
counter of 16 bits for the PWM.

### Digital circuit (digital_circuit.vhd)
preform the PWM signal using the counter.

---

Authors: Yagel ben zvi and ron benita